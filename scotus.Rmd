...
scdb_case %>%
  ...
  ggplot(mapping = aes(x = conserv, y = justiceName)) + 
  ###FIGURE OUT HOW TO ORDER BY SENIORITY y = reorder(justiceName, seniorjust)
...
```{r}
#gives me a graph but isn't colored
scdb_case %>%
  ...
  arrange(-yes_uncon) %>%
  slice(1:10, 71:80) %>%
  #do i change something w/ slice here?
...
```{r}
scdb_case %>%
  drop_na(term, decisionDirection, chief) %>%
  group_by(term, chief) %>%
  mutate(conserv_caselgl = (decisionDirection == 1)) %>%
  summarize(
    totalconserv_cases = n(),
...
#need to order justices by seniority. factor? or use justice (i.e. their ID #) & combine?
