...
scdb_case %>%
  ...
  mutate(type = ifelse(yes_uncon>0.90, "top", "bottom")) %>% #- YUP ASK FOR HELP HERE
  ggplot(mapping = aes(x = yes_uncon, y = fct_reorder(justiceName, .x = yes_uncon), fill = type)) + #would add fill = type to get color if mutate above worked
  geom_col() +
  scale_x_continuous(labels = scales::percent) +
...
scdb_case %>%
  ...
  drop_na(term, decisionDirection, Orderchief) %>%
   group_by(term, Orderchief) %>%
    mutate(Recode_decDirect = ifelse(decisionDirection != "1", 0, 1)) %>% #if it's liberal, it's 0. want if it's not conservative (aka liberal AND unspecficied), it's =
    summarize(
#    totalconserv_cases = n(),
