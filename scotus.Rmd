...
scdb_case %>%
  ...
  # select(caseIssuesId, direction, justice, justiceName, issueArea) %>%
  ...
  #group_by(issueArea) %>% --> NEED to group by justiceName, then again by issueArea
  summarize(
    totalvotes = n(),
...
#or maybe only grouping by issues after all & then using facet(JusticeName)?  
...
just_levels = c("JGRoberts", "CThomas", "SGBreyer", "SAAlito", "SSotomayor", "EKagan", "NMGorsuch", "BMKavanaugh", "ACBarrett")
#FYI won't have any entries for ACBarrett
scdb_vote %>%
mutate(senior = factor(justiceName, levels = just_levels))


mutate(seniority = factor(justiceName, levels = just_levels))
